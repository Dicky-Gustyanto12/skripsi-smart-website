...
export default function TableParameter() {
  ...
  const fetchKriteria = async () => {
    try {
      ...
      if (Array.isArray(result)) setKriteriaList(result);
      else if (result && Array.isArray(result.data))
        setKriteriaList(result.data);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:8000/api/parameter");
      let result = await res.json();
  ...
  const handleDelete = async (id_parameter) => {
    ...
    try {
      await fetch(`http://localhost:8000/api/parameter/${id_parameter}`, {
        method: "DELETE",
      });
      await fetchData();
